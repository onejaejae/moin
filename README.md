...
1. 확장성:
   새로운 통화가 추가되거나 기존 송금 견적서 생성 방식이 변경되더라도 기존 코드를 수정하지 않고, 새로운 전략 클래스를 추가하기만 하면 됩니다. 예를 들어, EUR 또는 GBP에 대한 송금 견적서 생성이 필요할 경우, 해당 통화에 대한 전략 클래스를 생성하고 팩토리에 추가하면 손쉽게 기능을 확장할 수 있습니다.

2. 유지보수성:
   각 통화의 송금 견적서 생성 로직이 독립적으로 관리되므로, 특정 통화 로직을 수정해도 다른 통화 로직에 영향을 주지 않습니다. 또한, 공통 로직(예: 환율 조회, 수수료 계산 등)은 별도의 서비스(ExchangeRateService, FeeService)로 분리해 재사용성을 높였습니다.
...
송금 접수 요청 기능에서 `일일 송금 한도 검증`과 같은 정책 관련 로직을 `requestTransfer` 서비스에 포함할 경우, 코드가 복잡해지고, 송금 접수 요청 처리와 송금 한도 검증이라는 두 가지 책임이 혼재되어 가독성과 유지보수성이 저하될 우려가 있다고 판단했습니다.
